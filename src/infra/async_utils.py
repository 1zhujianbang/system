...
【实体输出约束】
1. 实体必须“原子化”：一个实体名称只能指代一个主体或一个明确职务/机构，不要把多个主体/称谓粘连在同一个实体字符串里。
2. 若新闻以“职务/机构 + 姓名/简称”共同出现（如“美国总统特朗普”“英国安全大臣××”），必须拆分输出为两个实体（如“美国总统”“特朗普”）；不要输出粘连形式。实体关系与语义角色会在后续自动匹配两者。
3. 若同一主体出现多种表述，entities 用你认为最规范、最不易歧义的主名称；entities_original 用对应的原文表述逐一对齐。
4. 不要输出泛称、代词、集合性称呼、匿名指代（如“消息人士”“监管机构”“某公司”）。
...
【摘要要求】
abstract 必须尽可能具体、可判重、可长期复用：
- 必须包含关键主体（谁）与关键动作（做了什么/说了什么/决定了什么）以及关键客体（对谁/对什么）。
- 如新闻包含时间/地点/金额/数量/标的等要素，能写则写入摘要以提升唯一性。
- 禁止使用空泛表述（如“引发关注”“发生争议”“最新进展”“市场波动”）作为摘要主体。

【事件主名称（abstract）生成规则】
abstract 用作事件的“主事件名/唯一标识”，需满足：
1. 至少包含 entities 中的一个实体名称（原样出现）。
2. 必须包含一个明确的动作/言语谓词（如“起诉”“制裁”“宣布”“签署”“否认”“批准”“收购”“调查”“警告”“指责”“会见”“通过”等），避免只写“发生/出现/进展/引发关注”等弱谓词。
3. 尽量包含关键要素：金额/数量/标的/地点/时间（若文中给出且不增加歧义）。
4. 避免不确定与传闻化写法（如“据悉/传闻/可能/或将”）除非新闻明确以该方式表述且属于事件事实的一部分。
5. 尽量控制在 12–45 个中文字符内（过短会过于泛化，过长会降低复用性）。

示例（仅示意格式）：
- ✅ “美国证券交易委员会起诉某加密交易所涉嫌违规”
- ✅ “欧盟委员会批准某公司收购某公司交易”
- ❌ “市场出现波动”
- ❌ “最新进展：相关人士回应”
